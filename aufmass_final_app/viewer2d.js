...
let state = {
  ...
  depth:    0.73,
  sections: [],
  technik: { lastklasse: '', breitenklasse: '', verwendungszweck: [] },
  pdfTechnikToggles: { lastklasse: false, breitenklasse: false, verwendungszweck: false }
};
...
function openAddSheet() {
  ...
  sheet.innerHTML = `
    ...
    <div class="sheet-section-label">Schrittweite</div>
    <div class="sheet-step-row" id="sheetStepRow">
      <button class="step-btn${adjStep===0.25?' active':''}" data-step="0.25">0.25 m</button>
      <button class="step-btn${adjStep===0.5?' active':''}"  data-step="0.5">0.50 m</button>
      <button class="step-btn${adjStep===1.0?' active':''}"  data-step="1.0">1.00 m</button>
      <input type="number" class="step-custom-inp" id="sheetStepCustom" placeholder="Eigene" min="0.01" step="0.01" inputmode="decimal" />
    </div>
  ...
  // Step size buttons
  const stepCustomInp = document.getElementById('sheetStepCustom');
  sheet.querySelectorAll('.step-btn').forEach(btn => {
    btn.addEventListener('click', () => {
      adjStep = parseFloat(btn.dataset.step);
      sheet.querySelectorAll('.step-btn').forEach(b => b.classList.toggle('active', b === btn));
      stepCustomInp.value = '';
    });
  });
  stepCustomInp.addEventListener('input', () => {
    const v = parseFloat(stepCustomInp.value);
    if (v > 0) {
      adjStep = +v.toFixed(2);
      sheet.querySelectorAll('.step-btn').forEach(b => b.classList.remove('active'));
    }
  });

  document.getElementById('sheetMinus').addEventListener('click', () => {
    const v = parseFloat(customInp.value) || 2.57;
    selectLen(Math.max(0.25, +(v - adjStep).toFixed(2)));
  });
  document.getElementById('sheetPlus').addEventListener('click', () => {
    const v = parseFloat(customInp.value) || 2.57;
    selectLen(+(v + adjStep).toFixed(2));
  });
...
function openEditSheet(si, bi) {
  ...
  minusBtn.addEventListener('click', () => { bay.len = Math.max(0.25, +(bay.len - adjStep).toFixed(2)); syncInp(); });
  plusBtn.addEventListener('click',  () => { bay.len = +(bay.len + adjStep).toFixed(2); syncInp(); });
  ...
  stepRow.className = 'sheet-step-row';
  const STEPS = [0.25, 0.5, 1.0];
  STEPS.forEach(s => {
    const sb = document.createElement('button');
    sb.className = 'step-btn' + (adjStep === s ? ' active' : '');
    sb.textContent = s.toFixed(2) + ' m';
    sb.addEventListener('click', () => {
      adjStep = s;
      stepRow.querySelectorAll('.step-btn').forEach(b => b.classList.toggle('active', b === sb));
      stepCustInp.value = '';
    });
    stepRow.appendChild(sb);
  });
  const stepCustInp = document.createElement('input');
  stepCustInp.type = 'number'; stepCustInp.className = 'step-custom-inp';
  stepCustInp.placeholder = 'Eigene'; stepCustInp.min = '0.01'; stepCustInp.step = '0.01';
  stepCustInp.inputMode = 'decimal';
  stepCustInp.addEventListener('input', () => {
    const v = parseFloat(stepCustInp.value);
    if (v > 0) {
      adjStep = +v.toFixed(2);
      stepRow.querySelectorAll('.step-btn').forEach(b => b.classList.remove('active'));
    }
  });
  stepRow.appendChild(stepCustInp);
...
function renderSections() {
  ...
  state.sections.forEach((sec, si) => {
    ...
    sec.bays.forEach((bay, bi) => {
      ...
      lbl.className = 'bay-lbl'; lbl.textContent = 'L';
      ...
      row.appendChild(num); row.appendChild(lbl); row.appendChild(inp); row.appendChild(qd); row.appendChild(rmBay);
  ...
  if (state.sections.length > 0) {
    ...
    wrap.className = 'add-next-sec-wrap';
    const nextBtn = document.createElement('button');
    nextBtn.className = 'add-next-sec-btn';
    nextBtn.textContent = '+ Nächste Seite';
    nextBtn.addEventListener('click', () => { addCtx = null; openAddSheet(); });
...
function savePlan() {
  state.technik = {
    lastklasse:      document.getElementById('techLastklasse')?.value || '',
    breitenklasse:   document.getElementById('techBreitenklasse')?.value || '',
    verwendungszweck: [...document.querySelectorAll('.vz-btn.active')].map(b => b.dataset.vz)
  };
  state.pdfTechnikToggles = {
    lastklasse:      document.getElementById('pdfTLK')?.dataset.active === '1',
    breitenklasse:   document.getElementById('pdfTBK')?.dataset.active === '1',
    verwendungszweck:document.getElementById('pdfTVZ')?.dataset.active === '1'
  };
...
async function exportPdf() {
  ...
  const margin = 10;
  const tk  = state.technik || {};
  const ptt = state.pdfTechnikToggles || {};
  const vzArr = Array.isArray(tk.verwendungszweck) ? tk.verwendungszweck : [];
  const techLines = [
    (ptt.lastklasse    && tk.lastklasse)    ? `Lastklasse: ${tk.lastklasse}` : '',
    (ptt.breitenklasse && tk.breitenklasse) ? `Breitenklasse: ${tk.breitenklasse}` : '',
    (ptt.verwendungszweck && vzArr.length)  ? `Verwendungszweck: ${vzArr.join(', ')}` : ''
  ].filter(Boolean);
  const titleH = 20 + (techLines.length > 0 ? 2 + techLines.length * 4.5 : 0);
  ...
  if (techLines.length > 0) {
    ...
    doc.text('Technik:', margin, ty);
    doc.setFont('helvetica', 'normal');
    techLines.forEach(line => { ty += 4.5; doc.text(line, margin + 4, ty); });
  }
...
function syncTechnikUI() {
  const tk  = state.technik  || {};
  const ptt = state.pdfTechnikToggles || {};
  const lkEl = document.getElementById('techLastklasse');
  const bkEl = document.getElementById('techBreitenklasse');
  if (lkEl) lkEl.value = tk.lastklasse  || '';
  if (bkEl) bkEl.value = tk.breitenklasse || '';
  const vzList = Array.isArray(tk.verwendungszweck) ? tk.verwendungszweck : [];
  document.querySelectorAll('.vz-btn').forEach(btn => {
    const on = vzList.includes(btn.dataset.vz);
    btn.classList.toggle('active', on);
  });
  [['pdfTLK','lastklasse'],['pdfTBK','breitenklasse'],['pdfTVZ','verwendungszweck']].forEach(([id, key]) => {
    const btn = document.getElementById(id);
    if (!btn) return;
    btn.dataset.active = ptt[key] ? '1' : '0';
    btn.classList.toggle('active', !!ptt[key]);
  });
}
...
function init() {
  ...
  // Technik selects
  document.getElementById('techLastklasse').addEventListener('change', e => {
    state.technik.lastklasse = e.target.value;
  });
  document.getElementById('techBreitenklasse').addEventListener('change', e => {
    state.technik.breitenklasse = e.target.value;
  });

  // Verwendungszweck toggles
  document.querySelectorAll('.vz-btn').forEach(btn => {
    btn.addEventListener('click', () => {
      btn.classList.toggle('active');
      state.technik.verwendungszweck = [...document.querySelectorAll('.vz-btn.active')].map(b => b.dataset.vz);
    });
  });
  ...
  [['pdfTLK','lastklasse'],['pdfTBK','breitenklasse'],['pdfTVZ','verwendungszweck']].forEach(([id, key]) => {
    ...
    btn.addEventListener('click', () => {
      ...
      state.pdfTechnikToggles[key] = nowActive;
    });
  });
